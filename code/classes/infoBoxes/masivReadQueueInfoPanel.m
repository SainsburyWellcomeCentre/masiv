classdef masivReadQueueInfoPanel<handle
    ...
    methods
        function obj=masivReadQueueInfoPanel(parentFig, position, gzvm)
...
function updateNumberOfReadsInQueue(~,~, obj)
    ...
    frac=n/masivSetting('readQueueInfoPanel.max'); if frac>1; frac=1;end
...
function changeMax(~,~)
    ...
    if isempty(waitFlag)
        ...
        oldMax=masivSetting('readQueueInfoPanel.max');
        ...
        if ~isempty(newMax)&&~isempty(str2double(newMax))
            masivSetting('readQueueInfoPanel.max', str2double(newMax))
        end
    end
end
