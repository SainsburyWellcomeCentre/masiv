classdef masivCropper<masivPlugin
    ...
    methods
    function obj=masivCropper(caller, ~)
            ...
            try
                pos=masivSetting('cropper.figurePosition');
            catch
                ssz=get(0, 'ScreenSize');
                lb=[ssz(3)/3 ssz(4)/3];
                pos=round([lb 400 550]);
                masivSetting('cropper.figurePosition', pos)
            end
            %% Main UI
            obj.hFig=figure(...
                'Position', pos, ...
                ...
                'MenuBar', 'none', ...
                'NumberTitle', 'off', ...
                'Name', ['Cropper: ' obj.MaSIV.Meta.stackName], ...
    ...
    function updatePosition(obj, p,~)
        ...
        obj.hXSize.String=sprintf('%u (Original %u)', round(p(3)), round(obj.MaSIV.mainDisplay.imageXLimOriginalCoords(end)));
        obj.hYSize.String=sprintf('%u (Original %u)', round(p(4)), round(obj.MaSIV.mainDisplay.imageYLimOriginalCoords(end)));
        ...
        nPixelsOrig=round(obj.MaSIV.mainDisplay.imageXLimOriginalCoords(end))*round(obj.MaSIV.mainDisplay.imageYLimOriginalCoords(end));
        if nPixelsOrig<1e7
